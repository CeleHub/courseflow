...
export default function LecturersPage() {
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true)
      const baseParams = { page: 1, limit: 50, departmentCode: isHod ? user?.departmentCode : undefined }
      const [lecturersRes, hodsRes] = await Promise.all([
        apiClient.getUsers({ ...baseParams, role: Role.LECTURER } as any),
        apiClient.getUsers({ ...baseParams, role: Role.HOD } as any),
      ])
      const lecturersResult = getItemsFromResponse<UserType>(lecturersRes)
      const hodsResult = getItemsFromResponse<UserType>(hodsRes)
      const lecturers = lecturersResult?.items ?? []
      const hods = hodsResult?.items ?? []
      const merged = [...lecturers, ...hods]
  ...
  }, [isAdmin, isHod, user?.departmentCode])
  ...
  if (!isAdmin && !isHod) {
    return (
      <div className="text-center py-16">
        ...
        <p className="text-muted-foreground">You need admin or HOD privileges to access this page.</p>
      </div>
    )
  }
      ...
      <div className="space-y-8">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            View lecturers and heads of department
          </p>
        </div>
